...
import { updateCourseInPHDCS, fetchAllPHDCS } from '../../redux/actions/phdcs.action';
...
const UpdateCoursePopup = ({ setShowPopup, editingCourse, setEditingCourse, setRoadmap }) => {
    ...
    const handleSubmit = async () => {
        try {
            ...
            const response = await dispatch(updateCourseInPHDCS({ roadmapId, courseId, courseData: courseDetails }));

            if (response.type === 'phdcs/updateCourse/fulfilled') {
                const fetchResponse = await dispatch(fetchAllPHDCS());
                if (fetchResponse.type === 'phdcs/fetchAll/fulfilled') {
